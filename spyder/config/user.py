...
class UserConfig(DefaultsConfig):
        ...
        for section in self.sections():
            ...
            for option, value in self.items(section, raw=self._raw):
                try:
                    ...
                    pass
                sec_dict[option] = value
            self.defaults.append((section, sec_dict))
    ...
    def get(self, section, option, default=NoDefault):
        ...
        ----------
        section: str
            Section name. If `None` is provided use the default section name.
        option: str
...
class SpyderUserConfig(UserConfig):
    ...
    def apply_configuration_patches(self, old_version=None):
        ...
        if old_version and check_version(old_version, '44.1.0', '<'):
            run_lines = str(self.get(
                'ipython_console', 'startup/run_lines'))
        ...
        for name, sec_opts in name_map.items():
            ...
            if len(sec_opts) == 0:
                ...
                for section, options in sec_opts:
                    if len(options) == 0:
                        # Use all on section
                        sec = cls._get_section_from_defaults(
                            defaults_copy, section)
                        # Remove section from defaults
                        cls._remove_section_from_defaults(
                            defaults_copy, section)
                    else:
                        # Iterate and pop!
                        sec = {}
...
class PluginMultiConfig(MultiUserConfig):
    """Plugin configuration handler with multi-file support."""
